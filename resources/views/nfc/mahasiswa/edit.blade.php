...
@section('title', 'Edit Mahasiswa - Absensi NFC')

@section('content')
<div class="page-header">
    <h3 class="page-title">Edit Mahasiswa</h3>
    ...
    <div class="col-lg-8 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('nfc.mahasiswa.update', $mahasiswa) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        <label for="nfc_serial">NFC Serial</label>
                        <input type="text" name="nfc_serial" id="nfc_serial" class="form-control @error('nfc_serial') is-invalid @enderror" value="{{ old('nfc_serial', $mahasiswa->nfc_serial) }}" placeholder="Kosongkan jika belum ada kartu">
                        @error('nfc_serial')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Serial number kartu NFC. Biarkan kosong jika tidak diubah.</small>
                    </div>

                    <div class="form-group">
                        <label for="foto">URL Foto</label>
                        <input type="text" name="foto" id="foto" class="form-control @error('foto') is-invalid @enderror" value="{{ old('foto', $mahasiswa->foto) }}" placeholder="URL foto mahasiswa">
                        @error('foto')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="kelases[]" id="kelases" class="form-control select2" multiple="multiple">
                            @foreach($kelases as $kelas)
                                <option value="{{ $kelas->id }}" {{ $mahasiswa->kelases->contains($kelas->id) ? 'selected' : '' }}>
